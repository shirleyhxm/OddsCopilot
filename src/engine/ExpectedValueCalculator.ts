import { ExpectedValueResult } from '../types';
...
export class ExpectedValueCalculator {
   ...
   */
  static calculate(scenario: Scenario): ExpectedValueResult {
   ...
   */
  private static assessConfidence(scenario: Scenario): 'high' | 'medium' | 'low' {
   ...
   */
  private static generateWarnings(scenario: Scenario): string[] {
    ...
    if (Math.abs(totalProb - 1) > 0.1) {
      warnings.push(
        `Probabilities sum to ${(totalProb * 100).toFixed(1)}%, not 100%. ` +
    ...
    const dominantOutcomes = scenario.outcomes.filter(
      o => Math.abs(o.getContribution() / ev) > 0.5
    );
    if (dominantOutcomes.length > 0 && ev !== 0) {
      warnings.push(
        `One outcome (${dominantOutcomes[0].name}) drives >50% of the expected value. ` +
    ...
    const tailRisks = scenario.outcomes.filter(
      o => o.probability < 0.1 && Math.abs(o.value) > Math.abs(ev) * 2
    );
    if (tailRisks.length > 0) {
      warnings.push(
        `You have low-probability, high-impact outcomes. ` +
  ...
  ): { original: number; adjusted: number; delta: number } {
