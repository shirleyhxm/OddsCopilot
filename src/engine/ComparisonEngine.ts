...
export class ComparisonEngine {
   ...
   */
  static compare(decision: Decision): ComparisonResult {
  ...
  private static generateInsights(
    ...
    if (evRange === 0) {
      ...
      insights.push(
        `${best.name} has a significantly higher expected value (+${this.formatPercentage(evDiff / Math.abs(sorted[1].expectedValue))})`
      );
    } else if (evDiff / Math.abs(best.expectedValue) < 0.1) {
      insights.push(
        `The top options are very close in expected value (within ${this.formatPercentage(evDiff / Math.abs(best.expectedValue))}). ` +
    ...
    if (lowConfidence.length > 0) {
      insights.push(
        `${lowConfidence.map(s => s.name).join(', ')} ${lowConfidence.length === 1 ? 'has' : 'have'} low confidence. ` +
    ...
    if (withWarnings.length > 0) {
      insights.push(
        `Review the warnings for ${withWarnings.map(s => s.name).join(', ')}. ` +
        `They highlight important considerations about your assumptions.`
      );
    }

    // Check if best option has tail risks
    if (best.result.warnings?.some(w => w.includes('tail risks') || w.includes('low-probability'))) {
      insights.push(
        `The highest EV option has tail risks. If you're risk-averse, you might prefer a lower-EV, lower-variance option.`
      );
    }
   ...
   */
  private static generateSensitivityNotes(decision: Decision): string[] {
    const notes: string[] = [];
    ...
    decision.scenarios.forEach(scenario => {
      ...
      const dominantOutcomes = scenario.outcomes.filter(
        o => Math.abs(o.getContribution() / ev) > 0.4
      );

      if (dominantOutcomes.length > 0) {
        notes.push(
          `${scenario.name}: Most sensitive to your estimate of "${dominantOutcomes[0].name}" ` +
  ...
  ): Array<{ scenario: string; original: number; adjusted: number; delta: number }> {
    return decision.scenarios.map(scenario => {
      try {
        const analysis = ExpectedValueCalculator.sensitivityAnalysis(
          scenario,
          outcomeName,
          probabilityDelta
        );
